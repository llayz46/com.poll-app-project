...
if (isset($_POST['savePoll'])) {
  ...
  if ($res) {
    header('Location: add_modificate_sondage.php?id='.$res);
  } else {
    $pollError = 'Une erreur est survenue lors de la création du sondage';
  }
}
...
<form method="post">
  ...
  <div class="mb-3">
    ...
    <textarea name="description" id="description" cols="30" rows="5" class="form-control"><?=$poll['description']?></textarea>
  </div>
  ...
  <div class="mb-3">
    <input type="submit" name="savePoll" class="btn btn-primary" value="Créer le formulaire">
  </div>
</form>

<?php if(!isset($_GET['id'])) { ?>
  <div class="alert alert-warning" role="alert">
    Après avoir créé le sondage, vous pourrez ajouter des propositions.
  </div>
  <?php } else { ?>
  <div class="row m-4">
    <table class="table">
